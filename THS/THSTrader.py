...
class THSTrader:
    ...
    def get_balance(self):
        ...
        for key, control_id in BALANCE_CONTROL_ID_GROUP.items():
            result[key] = float(
                self.main_wnd.window(control_id=control_id,
...
#        print(cancelable_entrusts)
    ...
    def __trade(self, stock_no, price, amount):
        print(stock_no, price, amount)
        time.sleep(0.2)
        self.main_wnd.window(control_id=0x408, class_name="Edit").type_keys(
            str(stock_no))  # 设置股票代码
        time.sleep(0.5)
        # self.main_wnd.window(control_id=0x409, class_name="Edit").type_keys(
        #     str(price))  # 设置价格
        # time.sleep(0.5)
        self.main_wnd.window(control_id=0x40A, class_name="Edit").type_keys(
            str(int(amount)))  # 设置股数目
        time.sleep(0.2)
        self.main_wnd.window(
            control_id=0x3EE, class_name="Button").click()   # 点击卖出or买入
    ...
    def __get_grid_data(self, is_entrust=False):
        """ 获取grid里面的数据 """
        time.sleep(0.1)
        grid = self.main_wnd.window(
            control_id=0x417, class_name='CVirtualGridCtrl')
        time.sleep(0.1)
        ...
        while 0:
            ...
            self.app.top_window().window(
                control_id=0x964, class_name='Edit').set_text(captcha_num)  # 模拟输入验证码

            self.app.top_window().set_focus()
            keyboard.SendKeys("{ENTER}")   # 模拟发送enter，点击确定
            try:
                print(self.app.top_window().window(
                    control_id=0x966, class_name='Static').window_text())
            except:
                break
            time.sleep(0.1)
    ...
    def __cancel_by_double_click(self, row):
        ...
        y = 30 + 16 * row
        self.app.top_window().window(
            control_id=0x417, class_name='CVirtualGridCtrl').double_click(coords=(x, y))
